...
char** read_terrain(const char* nomfichier, int *nblig, int *nbcol)
{
  ...
  *nblig= 0;
  while(!feof(fi)) {
    ...
    chOld = ch;
    ch = fgetc(fi);
    if (ch!= '\n' && ch!= ' ' && ch!= '\r' && ch!= '#'){//if there are characters other than[space],[next line], and #
      printf("\nEROR!!!, the source file contains unknown characters, please verify the file: pentominos.txt\n\n");
      exit(0);
    }
    if(ch == '\n') {
      *nblig= *nblig +1;
...
void read_objects_v2(const char* nomfichier, int* nbobj, obj_t* tab_obj, SDL_Renderer *renderer)
{
  ...
  *nbobj = -1;
  ...
  SDL_Surface* tile = SDL_LoadBMP("./src/tile32.bmp");
  SDL_Surface* tile_hi = SDL_LoadBMP("./src/tile32hi.bmp");
...
void mouse_trigger(SDL_Point* p,obj_t *tab_obj, int* GLOBAL_OBJ_SELECTED, int* nbobj,int* dx, int* dy, int* GLOBAL_LAST_OBJ_SELECTED,SDL_Rect* cw_rect,SDL_Rect* ccw_rect, SDL_Rect* flip_rect, SDL_Rect* save_rect, SDL_Rect* load_rect,int* time_shift, long int* chronometer_counter, SDL_Window* win, SDL_Point* previous_position)
{
  ...
  if(*GLOBAL_OBJ_SELECTED == -1){//if no obj is selected
    if(myPointInRect(p, cw_rect)){//if icon of cw is hit
      ...
      load_button(tab_obj,time_shift,nbobj);
    }
  }
}
...
{ 
  ...
  if (*chronometer_counter == 0){//the first call of the function>>>it has to create texture
    ...
    *seconds_passed = (*chronometer_counter/1000);// chronometer counter count time in milli second and from that we extract each digit of chronometre up to 99:59
    ...
    digit1 = (*seconds_passed)%10;
    digit2 = (*seconds_passed/10)%6;
    digit3 = (*seconds_passed/60)%10;
    digit4 = (*seconds_passed/600)%10;
    str_time[5] = ' ';
    str_time[4] = '0'+digit1;
    str_time[3] = '0'+digit2;
    str_time[2] = ':';
...
void save_button(obj_t *tab_obj,int chronometer_counter, int nbobj)
{
  ...
  int obj,lig,col;
  FILE *fi;
  fi = fopen("./data/saved_parameters.txt","w");
  ...
  fputs("\n",fi);
  for(obj=0; obj<nbobj; obj++){
    fprintf(fi,"%03d ",tab_obj[obj].tab_rect[0][0].x);//print x coords
    fprintf(fi,"%03d",tab_obj[obj].tab_rect[0][0].y);//print y coords
    fputs("\n",fi);
  }
  fputs("/\n",fi);
  for(obj=0; obj<nbobj; obj++){
...
{
  //every parameter that has been saved will be reloaded as well as time
  int char_count=0,temp1=0,obj=0,lig=0,col=0, decimal_power=0, word_count=0;
  bool ori=false;
  FILE *fi;
  fi = fopen("./data/saved_parameters.txt","r");
  char ch;
  if(fi){//if file exists
    while(!feof(fi)) {//while its not end of the file
      char_count++;
      ch = fgetc(fi);
      //load time
      if(char_count>=1 && char_count<=4){//the first 4 digits represent time passed in seconds
        temp1 +=(ch-'0')*pow(10,(4-char_count));//converting digits to integers using power of 10
        if(char_count==4){
          *time_shift=temp1 ;//save pointer of time shift when
          printf("time shift pointer =%d\n",*time_shift);
          temp1=0;//reset to zero
        }
        
      }
      //load nbobj
      if(char_count==6 ||char_count==7){//the 2 digits which represent nbobj
        temp1 +=(ch-'0')*pow(10,(7-char_count));//converting digits to integers using power of 10
        if(char_count==7){
          *nbobj=temp1;//save pointer of time shift when
          printf("nbobj pointer in func load button=%d\n",*nbobj);
          temp1=0;//reset to zero
        }
      }
      //load position of rects
      if(char_count>=9 && obj<*nbobj && !ori){//from next line of nbobj counting each line as an obj until we load position for all objects
        if(ch=='\n'){//when reach next line character count(increment) an object
          for(lig=0; lig<5; lig++){
            for(col=0; col<5; col++){
              tab_obj[obj].tab_rect[lig][col].x=tab_obj[obj].tab_rect[0][0].x+32*col;
              tab_obj[obj].tab_rect[lig][col].y=tab_obj[obj].tab_rect[0][0].y+32*lig;
            }
          }
          obj++;
          decimal_power=0;
        ...
        }else if(ch==' '){//if ch equals space then we increment word_count
          word_count++;
          decimal_power=0;
          temp1=0;
        }else{
          if(word_count==0){//reading value of x
            temp1+=(ch-'0')*pow(10,(2-decimal_power));
            tab_obj[obj].tab_rect[0][0].x=temp1;
          }else if(word_count==1){//reading value of y
            temp1+=(ch-'0')*pow(10,(2-decimal_power));
            tab_obj[obj].tab_rect[0][0].y=temp1;
          }
          decimal_power++;
        }
      }
      //load orientation of objects (fill tab cell)
      if(ch=='/'){
        ori = true;
        obj = -1;
        lig=0;
      ...
      if(ori && obj < *nbobj){
        if (ch=='\n'){
          ...
          tab_obj[obj].tab_cell[lig][col]=-1;
        }else if(ch=='0'|| ch=='1' || ch=='2'){
          tab_obj[obj].tab_cell[lig][col]=ch-'0';
        }
  ...
  fclose(fi);
  *time_shift= *time_shift-(int)(SDL_GetTicks()/1000);
}
 
void gameover(long int* chronometer_counter,int* won,bool* GLOBAL_MENU , SDL_Window* window)
{
  if(*chronometer_counter>600000 && *won ==0){//if 10 minutes already passed and the player has not won the game so far
    SDL_ShowSimpleMessageBox(0,"GAMEOVER!!!","YOU HAVE PASSED 10 MINUTES AND NOT COMPLETED THE SHAPED WITH OBJEST...",window);
...
{
  //this function will determine if the player has won the game or not
  int lig,col,nb_cell,nb_cell_filled,lig2,col2,obj; 
  SDL_Rect dest;
  dest.x = p->x;
  dest.y = p->y;
  dest.w = 32;
  ...
  for(lig=0; lig<nblig; lig++){
    for(col=0; col<nbcol; col++){
      if(tab[lig][col]=='x'){
        ...
        nb_cell++;
        for(obj=0; obj<nbobj; obj++){
          for(lig2=0; lig2<5; lig2++){
            for(col2=0; col2<5 ;col2++){
              if(tab_obj[obj].tab_cell[lig2][col2]>=0){
                if(SDL_HasIntersection(&dest, &tab_obj[obj].tab_rect[lig2][col2])){
                  nb_cell_filled++;
                }
              }
            }
  ...
  if (nb_cell==nb_cell_filled && *won==0){
    *score=600-*seconds_passed;
    *won =1;
    *GLOBAL_MENU = true;
  }
}
...
{
  //this function will serve the mouse meft click trigger while menu is shown
  if(myPointInRect(p,play_rect)){//play button clicked
    *GLOBAL_MENU = false;
    *time_shift = -(int)(SDL_GetTicks()/1000);
    read_objects_v2("./data/pentomino.txt", nbobj, tab_obj, renderer);
  }else if(myPointInRect(p,resume_rect)){//resume bbutton clicked
    load_button(tab_obj,time_shift,nbobj);
    *GLOBAL_MENU = false;
  }else if(myPointInRect(p,quit_rect)){//quit button clicked
    *GLOBAL_MENU = false;
    *terminer = true;
  }
}
...
{
  //a function that will determine the apprpriate texture to be shown regarding the button that is hovered by mouse and will change the value of GLOBAL_MENU_HIGHLIGHT that suits
  //-1default,0play,1resume,2quit button
  if(myPointInRect(p,play_rect)){
    *GLOBAL_MENU_HIGHLIGHT=0;
  }else if(myPointInRect(p,resume_rect)){
    *GLOBAL_MENU_HIGHLIGHT=1;
  }else if(myPointInRect(p,quit_rect)){
    *GLOBAL_MENU_HIGHLIGHT=2;
  }else{
    *GLOBAL_MENU_HIGHLIGHT=-1;
  }
}
...
{
  //this fonction will render score and cup image as background when executed
  SDL_Surface *sur;
  SDL_Texture* score_text;
  SDL_Rect rect;
  char str[] = "your score is:      out of 600";
  str[15] = (*score/100)+'0';//hundreds
  str[16] = (*score/10)%10+'0';//tens
  str[17] = (*score%10)+'0';//ones
  SDL_Color Color = {40, 70, 190, 0};
  sur = TTF_RenderText_Solid(font_score, str, Color);
  score_text = SDL_CreateTextureFromSurface(renderer, sur);
  rect.x = 350;
  rect.y = 580;
  rect.w = sur->w;
  rect.h = sur->h;
  SDL_RenderCopy(renderer, cup, NULL, NULL);
  SDL_RenderCopy(renderer, score_text, NULL, &rect);
