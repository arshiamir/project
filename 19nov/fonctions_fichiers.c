...
void mouse_release(obj_t* obj, int* GLOBAL_OBJ_SELECTED, SDL_Rect* terrain, obj_t *tab_obj, int nbobj, SDL_Window* win){
  ...
  if(selected_obj_collision(tab_obj, GLOBAL_OBJ_SELECTED, nbobj)){//test to verify if the selected object has collision with other objects in the gamme
    SDL_ShowSimpleMessageBox(0, "Object Collision", "Error! the selected object collides with another object, please reposition it.", win);
    return;
  }
...
void render_icons(SDL_Renderer* renderer, SDL_Texture* cw, SDL_Texture* ccw, SDL_Texture* flip_tex, SDL_Rect* rect_cw, SDL_Rect* rect_ccw, SDL_Rect* rect_flip, SDL_RendererFlip* flip_none, int* GLOBAL_ICON_ANIM, double* GLOBAL_ICON_ANGLE){
  //icon animation int value refers to the icons which the cursor is pointing at,-1=none, 0=cw and 1=ccw and 2=flip_icon
  SDL_Rect src ={.x= 0, .y = 0, .h =64, .w =64};
  
  SDL_Point center={.x= 32, .y=31};
  if(*GLOBAL_ICON_ANIM==-1){
  ...
  }else if(*GLOBAL_ICON_ANIM==1){//*GLOBAL_ICON_ANIM=1
      SDL_RenderCopy(renderer, cw ,&src, rect_cw);
      SDL_RenderCopy(renderer, flip_tex ,&src, rect_flip);
      *GLOBAL_ICON_ANGLE = *GLOBAL_ICON_ANGLE-0.5;//increment angle degree by 1 
      SDL_RenderCopyEx(renderer,ccw, &src, rect_ccw, *GLOBAL_ICON_ANGLE, &center, *flip_none);
  }else if(*GLOBAL_ICON_ANIM==2){//flip change color
    SDL_RenderCopy(renderer, cw ,&src, rect_cw);
    SDL_RenderCopy(renderer, ccw ,&src, rect_ccw);
